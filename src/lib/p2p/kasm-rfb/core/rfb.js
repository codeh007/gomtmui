/* biome-ignore-all lint: vendored KasmVNC upstream source */
/*
...
import { xvncButtonToMask } from "./mousebuttonmapper.js";
import initializePrinterRelay from "./output/printer.js";
import initializeSmartcardRelay from "./output/smartcard.js";
import { dragThreshold, isDesktop, isIOS, isMac } from "./util/browser.js";
import Cursor from "./util/cursor.js";
import { clientToElement } from "./util/element.js";
import { setCapture } from "./util/events.js";
import EventTargetMixin from "./util/eventtarget.js";
import { hashUInt8Array, toSigned32bit, toSignedRelative16bit, toUnsigned32bit } from "./util/int.js";
import * as Log from "./util/logging.js";
import { decodeUTF8, encodeUTF8 } from "./util/strings.js";
...
// Wheel thresholds
const WHEEL_LINE_HEIGHT = 19; // Pixels for one line step (on Windows)
...
export default class RFB extends EventTargetMixin {
  ...
  set pointerRelative(value) {
    ...
    if (value) {
      const max_w = this._display.scale === 1 ? this._fbWidth : this._fbWidth * this._display.scale;
      const max_h = this._display.scale === 1 ? this._fbHeight : this._fbHeight * this._display.scale;
  ...
  get statsDroppedFps() {
    return this._display.droppedFps;
  }
  ...
  get hwEncoderProfile() {
    return this._hwEncoderProfile;
  }
  ...
  refreshSecondaryDisplays() {
    ...
    if (this._display.screens.length > 1) {
      this._proxyRFBMessage("applySettings", [
        this._hiDpi,
        this._clipViewport,
        this._scaleViewport,
        this._resizeSession,
  ...
  applyScreenPlan(screenPlan) {
    if (this._isPrimaryDisplay) {
      const fullPlan = this._screenSize();
      ...
      const changes = this._display.applyScreenPlan(screenPlan);
  ...
  async clipboardPasteDataFrom(clipdata) {
    ...
    for (let i = 0; i < clipdata.length; i++) {
      for (let ti = 0; ti < clipdata[i].types.length; ti++) {
        const mime = clipdata[i].types[ti];

        switch (mime) {
          case "image/png":
          case "text/plain":
          case "text/html": {
            const blob = await clipdata[i].getType(mime);
            if (!blob) {
              continue;
            }
    ...
    if (mimes.includes("image/png") && !mimes.includes("text/plain")) {
      const i = mimes.indexOf("image/png");
  ...
  _setupWebSocket() {
    ...
    this._sock.on("open", () => {
      if (this.isConnecting && this._rfbInitState === "") {
    ...
    this._sock.on("error", (_e) => Log.Warn("WebSocket on-error event"));
  ...
  _connect() {
    ...
    if (typeof RTCPeerConnection !== "undefined" && this._isPrimaryDisplay) {
      ...
      peer.onicecandidate = (e) => {
        if (e.candidate) Log.Debug("received ice candidate", e.candidate);
        else Log.Debug("all candidates received");
      };
      ...
      const _sock = this._sock;
      const udpBuffer = this._udpBuffer;
      const me = this;
      this._udpChannel.onmessage = (e) => {
        //Log.Info("got udp msg", e.data);
        const u8 = new Uint8Array(e.data);
        // Got an UDP packet. Do we need reassembly?
        const id = parseInt(u8[0] + (u8[1] << 8) + (u8[2] << 16) + (u8[3] << 24), 10);
        const i = parseInt(u8[4] + (u8[5] << 8) + (u8[6] << 16) + (u8[7] << 24), 10);
        const pieces = parseInt(u8[8] + (u8[9] << 8) + (u8[10] << 16) + (u8[11] << 24), 10);
        const _hash = parseInt(u8[12] + (u8[13] << 8) + (u8[14] << 16) + (u8[15] << 24), 10);
  ...
  _handleVisibilityChange(_event) {
  ...
  _requestRemoteResize() {
    ...
    if (this._isPrimaryDisplay) {
      ...
      Log.Debug(`Requested new desktop size: ${size.serverWidth}x${size.serverHeight}`);
  ...
  _screenSize(limited) {
    return this._display.getScreenSize(
      this.videoQuality,
      this.forcedResolutionX,
      this.forcedResolutionY,
      this._hiDpi,
      limited,
      !this._resizeSession,
      this._streamMode,
    ...
    if (this._isPrimaryDisplay) {
      ...
      switch (event.data.eventType) {
        case "register": {
          ...
          const screenIndex = this._display.addScreen(
            event.data.screenID,
            event.data.width,
            event.data.height,
            event.data.pixelRatio,
            event.data.containerHeight,
            event.data.containerWidth,
            event.data.scale,
            event.data.serverWidth,
            event.data.serverHeight,
            event.data.x,
            event.data.y,
            event.data.windowId,
          );
          this._proxyRFBMessage("screenRegistrationConfirmed", [
            this._display.screens[screenIndex].screenID,
            screenIndex,
          ]);
          this._sendEncodings();
          clearTimeout(this._resizeTimeout);
        ...
        case "reattach": {
          const _changes = this._display.addScreen(
            event.data.screenID,
            event.data.width,
            event.data.height,
            event.data.pixelRatio,
        ...
        case "unregister":
          ...
          break;
        case "mousemove":
          coords = this._display.getServerRelativeCoordinates(
            event.data.screenIndex,
            event.data.args[0],
            event.data.args[1],
          );
  ...
  _handleSecondaryDisplayMessage(_event) {
    if (this._isPrimaryDisplay) {
    }
  }
  ...
  _handlePointerLockChange(_env) {
    ...
    this.dispatchEvent(new CustomEvent("inputlock", { detail: { pointer: this._pointerLock } }));
  }

  _handlePointerLockError() {
    this._pointerLockChanging = false;
    this.dispatchEvent(new CustomEvent("inputlockerror", { detail: { pointer: this._pointerLock } }));
  }
  ...
  _handleWheel(ev) {
    ...
    if (ev.ctrlKey && !this._keyboard._keyDownList.ControlLeft) {
      ...
      this._watchForPinchAndZoom =
        this._watchForPinchAndZoom ||
        setInterval(() => {
          const timeSinceLastPinchAndZoom = Date.now() - this._mouseLastPinchAndZoomTime;
    ...
    if (this._watchForPinchAndZoom) {
      this._mouseLastPinchAndZoomTime = Date.now();
    }
  ...
  _handleTapEvent(ev, bmask) {
    ...
    if (
      ...
      const dx = this._gestureFirstDoubleTapEv.detail.clientX - ev.detail.clientX;
      const dy = this._gestureFirstDoubleTapEv.detail.clientY - ev.detail.clientY;
  ...
  _handleGesture(ev) {
    ...
    const pos = clientToElement(ev.detail.clientX, ev.detail.clientY, this._canvas);
    switch (ev.type) {
      case "gesturestart":
  ...
  _negotiateTightAuth() {
    ...
    for (const authType in clientSupportedTypes) {
  ...
  _hasWebp() {
    ...
    if (match && parseInt(match[1], 10) >= 65) return true;
    match = uagent.match(/chrome\/([0-9]+)\./);
    if (match && parseInt(match[1], 10) >= 23) return true;
    return false;
  }
  ...
  _handleBinaryClipboard() {
    ...
    const num = this._sock.rQshift8(); // how many different mime types
    const mimes = [];
    const clipItemData = {};
    let buffByteLen = 2;
    let textdata = "";
    ...
    for (let i = 0; i < num; i++) {
      ...
      buffByteLen += 4;
      const clipid = this._sock.rQshift32();

      if (this._sock.rQwait("Binary Clipboard mimelen", 1, buffByteLen)) {
        return false;
      }
      buffByteLen++;
      const mimelen = this._sock.rQshift8();
      ...
      buffByteLen += 4;
      const len = this._sock.rQshift32();
  ...
  _handle_server_stats_msg() {
    ...
    Log.Debug(text);
    this.dispatchEvent(new CustomEvent("bottleneck_stats", { detail: { text: text } }));
  ...
  _handleUserJoin() {
    const length = this._sock.rQshift32();
    if (this._sock.rQwait("KASM Shared Session Join", length, 32)) {
      return false;
    }

    const text = this._sock.rQshiftStr(length);
    this.dispatchEvent(new CustomEvent("sharedSessionUserJoin", { detail: text }));
    return true;
  }

  _handleUserLeft() {
    const length = this._sock.rQshift32();
    if (this._sock.rQwait("KASM Shared Session Left", length, 32)) {
      return false;
    }

    const text = this._sock.rQshiftStr(length);
    this.dispatchEvent(new CustomEvent("sharedSessionUserLeft", { detail: text }));
    return true;
  }
  ...
  _handleUdpRect(data, frame_id) {
    ...
    switch (frame.encoding) {
      ...
      case encodings.encodingTight: {
        const decoder = this._decoders[encodings.encodingUDP];
        try {
  ...
  _handleUdpUpgrade() {
    ...
    peer
      .setRemoteDescription(new RTCSessionDescription(response.answer))
      .then(
        function () {
  ...
  _handleUnixRelay() {
    ...
    const namelen = this._sock.rQshift8();
    ...
    processRelay?.(payload);
  }

  _handleServerVideoEncoders() {
    if (this._sock.rQwait("VideoEncoders header", 1, 1)) return false;
    ...
    if (this._sock.rQwait("VideoEncoders data", num * 13, 1)) return false;

    const serverSupportedEncoders = [];
    ...
    this.dispatchEvent(
      new CustomEvent("videocodecschange", {
        detail: {
  ...
  _handleDisconnectNotify() {
    if (this._sock.rQwait("DisconnectNotify header", 8, 1)) {
      return false;
    }
    const flags = this._sock.rQshift8();
    this._sock.rQskipBytes(3);
    const reasonLength = this._sock.rQshift32();
    if (reasonLength > 0 && this._sock.rQwait("DisconnectNotify reason", reasonLength, 8)) {
      return false;
    }
  ...
  _handleVMwareCursor() {
    ...
    if (cursorType === 0) {
      ...
      const andMask = new Array(w * h);
      for (let pixel = 0; pixel < w * h; pixel++) {
        andMask[pixel] = this._sock.rQshift32();
      }

      const xorMask = new Array(w * h);
      for (let pixel = 0; pixel < w * h; pixel++) {
      ...
      for (let pixel = 0; pixel < w * h; pixel++) {
        const data = this._sock.rQshift32();
  ...
  _handleCursor() {
    ...
    const bytes = pixelslength + masklength;
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        const maskIdx = y * Math.ceil(w / 8) + Math.floor(x / 8);
        const alpha = (mask[maskIdx] << (x % 8)) & 0x80 ? 255 : 0;
  ...
  _handleExtendedDesktopSize() {
    ...
    for (let i = 0; i < numberOfScreens; i += 1) {
      ...
      const sI = this._sock.rQshift32(); // id
      const x = this._sock.rQshift16(); // width
      const y = this._sock.rQshift16(); // height
    ...
    if (this._display.screens.length > 1 && this._forceFullFrameUpdateAfterResize) {
      ...
      this._forceFullFrameUpdateTimeout = setTimeout(
        function () {
          RFB.messages.fbUpdateRequest(this._sock, false, 0, 0, this._fbWidth, this._fbHeight);
        }.bind(this),
        500,
  ...
  _handleDataRect() {
    ...
    try {
      if (
        this._transitConnectionState === this.TransitConnectionStates.Udp ||
        this._transitConnectionState === this.TransitConnectionStates.Failure
      ) {
...
RFB.messages = {
  ...
  extendedClipboardNotify(sock, formats) {
    const flags = RFB.messages._buildExtendedClipboardFlags([extendedClipboardActionNotify], formats);
    RFB.messages.clientCutText(sock, flags, true);
  },

  extendedClipboardRequest(sock, formats) {
    const flags = RFB.messages._buildExtendedClipboardFlags([extendedClipboardActionRequest], formats);
    RFB.messages.clientCutText(sock, flags, true);
  },
