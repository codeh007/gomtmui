...
class SmartcardSession {
  ...
  async _establishContext() {
    return await this._callExtension("establish_context", 0).then(([_status, context]) => context);
  }
  ...
  async _listReaders(context) {
    return await this._callExtension("list_readers", context).then(([_status, readers]) => {
  ...
  async _connect(context, reader) {
    return await this._callExtension("connect", context, 2, 3, reader).then(
      ([_status, _cardContext, cardHandle, activeProtocol]) => ({
        cardHandle,
        activeProtocol,
      }),
    );
  ...
  async _transmit(apdu) {
    return await this._callExtension("transmit", this.context, this.cardHandle, this.activeProtocol, toHex(apdu)).then(
      ([_status, _context, _card, _protocol, response]) => fromHex(response),
    );
  }
      ...
      switch (command) {
        ...
        case REQUEST_STATUS:
          ...
          sendSmartcardResponse(
            RESPONSE_ACK,
            clientSession.cardAtr ? fromHex(clientSession.cardAtr) : new Uint8Array(0),
          );
          break;
