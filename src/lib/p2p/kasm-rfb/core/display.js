...
export default class Display {
  constructor(target, isPrimaryDisplay) {
    ...
    this._frameStatsInterval = setInterval(
      function () {
        const delta = Date.now() - this._lastFlip;
        if (delta > 0) {
          this._fps = (this._flipCnt / (delta / 1000)).toFixed(2);
          if (this._droppedFrames > this._droppedFramesCnt) {
            const droppedFrames = this._droppedFrames - this._droppedFramesCnt;
        ...
        Log.Debug(
          `Dropped Frames: ${this._droppedFrames} Dropped Rects: ${this._droppedRects} Forced Frames: ${this._forcedFrameCnt} Missing Flips: ${this._missingFlipRect} Late Flips: ${this._lateFlipRect}`,
        );
  ...
  set enableCanvasBuffer(value) {
    ...
    if (value && this._target) {
      ...
      const saveImg = this._targetCtx.getImageData(0, 0, this._target.width, this._target.height);
      this._drawCtx.putImageData(saveImg, 0, 0);

      if (this._transparentOverlayImg) {
        this.drawImage(
          this._transparentOverlayImg,
          this._transparentOverlayRect.x,
          this._transparentOverlayRect.y,
          this._transparentOverlayRect.width,
          this._transparentOverlayRect.height,
          true,
        );
      }
  ...
  get droppedFps() {
    return this._droppedFramesRate;
  }
  ...
  getClientRelativeCoordinates(x, y) {
    for (let i = 0; i < this._screens.length; i++) {
      if (
        x >= this._screens[i].x &&
        x <= this._screens[i].x + this._screens[i].serverWidth &&
        y >= this._screens[i].y &&
        y <= this._screens[i].y + this._screens[i].serverHeight
      ) {
        return {
          screenIndex: i,
  ...
  getScreenSize(resolutionQuality, max_width, max_height, hiDpi, disableLimit, disableScaling, streamMode) {
    ...
    const parentNodeSize = this._target.parentNode.getBoundingClientRect();
    ...
    if (
      this._screens[i].serverReportedWidth > 0 &&
      this._screens[i].serverReportedHeight > 0 &&
      (disableScaling ||
    ...
    //force full resolution on a high DPI monitor where the OS is scaling
      ...
      _scale = 1 / this._screens[i].pixelRatio;
    }
    //physically small device with high DPI
      ...
      const scaledWidth = width * this._screens[i].pixelRatio * (1 / targetDevicePixelRatio);
    ...
    const clientServerRatioH = this._screens[i].containerHeight / height;
    const clientServerRatioW = this._screens[i].containerWidth / width;
  ...
  addScreen(
    ...
    if (screenIdx > 0) {
      ...
      } else Log.Debug(`Channel not found for screenId ${screenID}`);
  ...
  viewportChangeSize(width, height) {
    if (
      (!this._clipViewport && this._screens.length === 1) ||
  ...
  copyImage(oldX, oldY, newX, newY, w, h, frame_id, fromQueue) {
    if (!fromQueue) {
      ...
      const sourceCvs = this._enableCanvasBuffer ? this._backbuffer : this._target;
  ...
  imageRect(x, y, width, height, mime, arr, frame_id) {
    ...
    if (!rect.inSecondary) {
      if (typeof ImageDecoder !== "undefined" && this._threading) {
        const imageDecoder = new ImageDecoder({ data: arr, type: mime });
        rect.type = "vid";
      ...
      const src = `data: ${mime};base64,${Base64.encode(arr)}`;
  ...
  transparentRect(x, y, width, height, img, frame_id, hashId) {
    ...
    if (rect.inPrimary) {
      const imageBmpPromise = createImageBitmap(img);
      imageBmpPromise.then(
        function (bitmap) {
  ...
  blitImage(x, y, width, height, arr, offset, frame_id, fromQueue) {
    if (!fromQueue) {
      ...
      let data;
      if (!ArrayBuffer.isView(arr)) {
        data = new Uint8ClampedArray(arr, arr.length + offset, width * height * 4);
      } else {
        data = new Uint8ClampedArray(arr.buffer, arr.byteOffset + offset, width * height * 4);
      }
  ...
  blitQoi(x, y, width, height, arr, _offset, frame_id, fromQueue) {
    if (!fromQueue) {
      const rect = {
        type: "blitQ",
        data: arr,
  ...
  drawImage(img, x, y, w, h, _overlay = false) {
    try {
  ...
  drawVideoFrame(videoFrame, x, y, w, h) {
    try {
      const targetCtx = this._enableCanvasBuffer ? this._drawCtx : this._targetCtx;
  ...
  clearRect(x, y, width, height, _offset, frame_id, fromQueue) {
    if (!fromQueue) {
      const rect = {
        type: "clear",
        x: x,
        ...
        case "rect": {
          ...
          switch (rect.type) {
            ...
            case "_img":
              ...
              break;
            case "transparent": {
              const imageBmpPromise = createImageBitmap(rect.arr);
              imageBmpPromise.then(
                function (img) {
  ...
  _asyncRenderQPush(rect) {
    ...
    for (let i = 0; i < this._asyncFrameQueue.length; i++) {
      if (rect.frame_id === this._asyncFrameQueue[i][0]) {
        ...
        const rect_cnt = rect.type === "flip" ? rect.rect_cnt : 0;
    ...
    if (frameIx >= 0) {
      if (rect.type === "flip") {
        ...
        if (this._asyncFrameQueue[frameIx][1] !== 0) {
          Log.Warn(
            `Redundant flip rect, current rect_cnt: ${this._asyncFrameQueue[frameIx][1]}, new rect_cnt: ${rect.rect_cnt}`,
          );
        }
      ...
      if (
        this._asyncFrameQueue[frameIx][1] > 0 &&
        this._asyncFrameQueue[frameIx][2].length >= this._asyncFrameQueue[frameIx][1]
      ) {
        //frame is complete
  ...
  _clearAsyncQueue() {
    ...
    for (const frame of this._asyncFrameQueue) {
      for (const rect of frame[2]) if (rect.type === "video_frame") rect.frame?.close();
    }
    ...
    if (force) {
      ...
      while (currentFrameRectIx < this._asyncFrameQueue[frameIx][2].length) {
        if (this._asyncFrameQueue[frameIx][2][currentFrameRectIx].type === "img") {
          if (
            this._asyncFrameQueue[frameIx][2][currentFrameRectIx].img &&
            !this._asyncFrameQueue[frameIx][2][currentFrameRectIx].img.complete
          ) {
      ...
      while (currentFrameRectIx < this._asyncFrameQueue[frameIx][2].length) {
        if (
          this._asyncFrameQueue[frameIx][2][currentFrameRectIx].type === "img" &&
          !this._asyncFrameQueue[frameIx][2][currentFrameRectIx].img.complete
        ) {
          this._asyncFrameQueue[frameIx][2][currentFrameRectIx].img.addEventListener("load", () => {
            this._asyncFrameComplete(frameIx);
          });
  ...
  _pushAsyncFrame(force = false) {
    if (this._asyncFrameQueue[0][3] || force) {
      const frame = this._asyncFrameQueue[0][2];
      const frameId = this._asyncFrameQueue.shift()[0];
      ...
      for (let i = 0; i < frame.length; i++) {
        ...
        for (let sI = 0; sI < a.screenLocations.length; sI++) {
          const screenLocation = a.screenLocations[sI];
          ...
          if (secondaryScreenRects > 0 && this._lastTransparentRectId !== this._transparentOverlayRect.hash_id) {
            for (let sI = 1; sI < this._transparentOverlayRect.screenLocations.length; sI++) {
              if (this._screens[this._transparentOverlayRect.screenLocations[sI].screenIndex].channel) {
                this._screens[this._transparentOverlayRect.screenLocations[sI].screenIndex].channel.postMessage({
                  eventType: "rect",
      ...
      if (secondaryScreenRects > 0) {
        for (let i = 1; i < this.screens.length; i++) {
          if (this._screens[i].channel) {
            this._screens[i].channel.postMessage({
  ...
  _processRectScreens(rect) {
    ...
    if (rect.type === "video_frame") {
      ...
      for (let i = 0; i < this._screens.length; i++) {
        ...
        if (
          !(
            (rect.x > screen.x2 || screen.x > rect.x + rect.width) &&
            (rect.y > screen.y2 || screen.y > rect.y + rect.height)
          )
        ) {
          const screenPosition = {
            x: 0 - (screen.x - rect.x), //rect.x - screen.x,
  ...
  _rescale(factor) {
    ...
    Log.Info(
      `Pixel Ratio: ${window.devicePixelRatio}, VNC Scale: ${factor}VNC Res: ${vp.serverWidth}x${vp.serverHeight}`,
    );
    ...
    if (
      ...
      this.antiAliasing === 1 ||
      (this.antiAliasing === 0 && factor !== 1 && this._target.style.imageRendering !== "auto")
    ) {
