...
export default class KasmVideoDecoder {
  ...
  _configureDecoder(screen) {
    Log.Debug(
      "Configuring decoder for screen: ",
      screen.id,
      " codec: ",
      VIDEO_CODEC_NAMES[screen.codec],
      " width: ",
      screen.width,
      " height: ",
      screen.height,
  ...
  _processVideoFrameRect(screenId, codec, x, y, width, height, sock, _display, _depth, frame_id) {
    const [keyFrame, dataArr] = this._readData(sock);
    ...
    try {
      ...
      if (screen.pendingFrames?.length > 0) {
        for (const frame of screen.pendingFrames) screen.decoder.decode(frame);
      }

      screen.decoder.decode(vidChunk);
    } catch (e) {
      Log.Error(
        "DECODE FAILURE - Screen: ",
        screenId,
        "Key frame ",
        keyFrame,
        " frame_id: ",
  ...
  dispose() {
    for (const screen of this._decoders.values()) {
      screen.decoder.close();
    }
    this._decoders.clear();

    for (const timestamp of this._timestampMap.keys()) {
