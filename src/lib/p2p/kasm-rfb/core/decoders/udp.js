...
export default class UDPDecoder {
  ...
  _fillRect(x, y, width, height, data, display, _depth, frame_id) {
    display.fillRect(x, y, width, height, [data[13], data[14], data[15]], frame_id, this._directDraw);

    return true;
  }

  _jpegRect(x, y, width, height, data, display, _depth, frame_id) {
    const img = this._readData(data);
    if (img === null) {
      return false;
    }
  ...
  _webpRect(x, y, width, height, data, display, _depth, frame_id) {
    const img = this._readData(data);
    if (img === null) {
      return false;
    }
  ...
  _pngRect(_x, _y, _width, _height, _data, _display, _depth, _frame_id) {
  ...
  _copyFilter(streamId, x, y, width, height, data, display, _depth, frame_id, data_index = 14) {
  ...
  _paletteFilter(streamId, x, y, width, height, data, display, _depth, frame_id) {
    const numColors = data[14] + 1;
    const paletteSize = numColors * 3;
    const palette = data.slice(15, 15 + paletteSize);

    const bpp = numColors <= 2 ? 1 : 8;
    const rowSize = Math.floor((width * bpp + 7) / 8);
    const uncompressedSize = rowSize * height;
    const data_i = 15 + paletteSize;
  ...
  _gradientFilter(_streamId, _x, _y, _width, _height, _data, _display, _depth, _frame_id) {
