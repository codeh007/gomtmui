...
function createQoiWorker() {
  return new Worker(new URL("./qoi/decoder.js", import.meta.url), { type: "module" });
}
...
export default class TightDecoder {
  ...
  _fillRect(x, y, width, height, sock, display, _depth, frame_id) {
  ...
  _jpegRect(x, y, width, height, sock, display, _depth, frame_id) {
  ...
  _webpRect(x, y, width, height, sock, display, _depth, frame_id) {
    const data = this._readData(sock);
    if (data === null) {
      return false;
    }
  ...
  _processRectQ() {
    while (this._availableWorkers.length > 0 && this._qoiRects.length > 0) {
      const i = this._availableWorkers.pop();
      const worker = this._workers[i];
      const rect = this._qoiRects.shift();
  ...
  _qoiRect(x, y, width, height, sock, _display, depth, frame_id) {
    const data = this._readData(sock);
    if (data === null) {
      return false;
    }
    ...
    if (this._enableQOI) {
      ...
      const item = { x: x, y: y, width: width, height: height, data: dataClone, depth: depth, frame_id: frame_id };
  ...
  _itRect(x, y, width, height, sock, display, _depth, frame_id) {
    ...
    if (!(h === this._lastTransparentRectHash && info === this._lastTransparentRectInfo)) {
      ...
      // unpack
      const rgba = new Uint8Array(width * height * 4 + 4);
      ...
      const img = new ImageData(new Uint8ClampedArray(rgba.buffer, 0, width * height * 4), width, height);
  ...
  _pngRect(_x, _y, _width, _height, _sock, _display, _depth, _frame_id) {
  ...
  _copyFilter(streamId, x, y, width, height, sock, display, _depth, frame_id) {
  ...
  _paletteFilter(streamId, x, y, width, height, sock, display, _depth, frame_id) {
  ...
  _gradientFilter(_streamId, _x, _y, _width, _height, _sock, _display, _depth, _frame_id) {
  ...
  async _disableQOIWorkers() {
    if (this._workers) {
      ...
      for await (const i of Array.from(Array(this._threads).keys())) {
  ...
  _enableQOIWorkers() {
    const wasmUrl = new URL("./qoi/qoi_viewer_bg.wasm", import.meta.url).toString();
    ...
    for (let i = 0; i < this._threads; i++) {
      this._workers.push(createQoiWorker());
      this._workers[i].onmessage = (evt) => {
        this._availableWorkers.push(i);
        switch (evt.data.result) {
          case 0: {
            evt.data.freemem = null;
            const data = new Uint8ClampedArray(evt.data.data);
            const img = new ImageData(data, evt.data.img.width, evt.data.img.height, {
    ...
    for (let i = 0; i < this._threads; i++) {
      this._workers[i].postMessage({ wasmUrl });
    }
