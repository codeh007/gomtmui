...
import { resolveCanonicalBootstrapState } from "./bootstrap-truth";
...
describe("resolveCanonicalBootstrapState", () => {
  ...
  it("blocks a non-live override until explicitly allowed", () => {
    const result = resolveCanonicalBootstrapState({
      liveTruth: LIVE_TRUTH,
      storedBootstrapAddr: LIVE_TRUTH.candidates[0].addr,
      overrideBootstrapAddr: "/dns4/manual.example.com/tcp/443/tls/ws/p2p/12D3KooWManual",
      allowOverride: false,
    });

    expect(result.mode).toBe("blocked-override");
    expect(result.selected).toBeNull();
    expect(result.blockedOverrideBootstrapAddr).toBe(
      "/dns4/manual.example.com/tcp/443/tls/ws/p2p/12D3KooWManual",
    );
  });
});
