...
import { canStartAndroidHostDeviceService, canStopAndroidHostDeviceService, resolveAndroidHostRuntimeDevice, type AndroidHostRuntimeDevice, waitForPolledValue } from "@/components/devices/device-state";
import {
		...
		startTransition(async () => {
			try {
				...
				toast.success("当前 Android 宿主已绑定", {
					description: credentialVersion === null ? "设备已绑定；设备是否在线将由后续 heartbeat 自动决定。" : `设备已绑定，并签发运行凭据 v${credentialVersion}；设备是否在线将由后续 heartbeat 自动决定。`,
				});
				router.refresh();
			} catch (error) {
	...
	const handleStartDeviceService = () => {
		...
		startTransition(async () => {
			try {
				...
				if (!hostConfirmed) {
					throw new Error("宿主尚未确认设备服务已启动，未记录启动审计");
				}
				await activateAndroidHostDeviceAction({ deviceId: boundDeviceId });
				setActivationSurface(readAndroidActivationSurface());
				toast.success("已请求启动设备服务", {
					description: "已记录启动请求；设备是否在线将由后续 heartbeat 自动决定。",
				});
				router.refresh();
			} catch (error) {
				toast.error(error instanceof Error ? error.message : "启动设备服务后记录审计失败");
			}
		});
	};
	...
	const handleStopDeviceService = () => {
		...
		startTransition(async () => {
			try {
				...
				if (!hostConfirmed) {
					throw new Error("宿主尚未确认设备服务已停止，未记录停止审计");
				}
				...
				toast.success("已请求停止设备服务", {
					description: "已记录停止请求；若后续没有 heartbeat，设备会在超时后显示离线。",
				});
				router.refresh();
			} catch (error) {
				toast.error(error instanceof Error ? error.message : "停止设备服务后记录审计失败");
			}
		});
	};
		...
		<Card>
			...
			<CardContent className="space-y-4 text-sm">
				...
				<div className="rounded-md border bg-muted/30 p-3">
					...
					<div className="mt-1 text-xs text-muted-foreground">这里显示的是 Android 壳层是否已经收到“启动设备服务”的动作回执，不代表设备核心 runtime 已在线；真正的在线真相仍由后续 heartbeat 决定。</div>
				</div>

				{matchedDevice ? (
					<div className="rounded-md border bg-muted/30 p-3">
						<div className="text-muted-foreground">数据库中的当前设备记录</div>
						<div className="mt-2 text-xs text-muted-foreground">最近心跳：{matchedDevice.lastSeenAt ?? "-"}</div>
						<div className="mt-1 text-xs text-muted-foreground">最近错误：{matchedDevice.lastError ?? "-"}</div>
						<div className="mt-1 text-xs text-muted-foreground">已归档：{matchedDevice.archivedAt ? "是" : "否"}</div>
					</div>
				) : null}
